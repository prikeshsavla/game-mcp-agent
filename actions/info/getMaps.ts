...
import {
  API_ENDPOINTS,
  MapContentType,
  VALID_MAP_CONTENT_TYPES,
} from "../../utils/actions/constants.ts";
...
async function getMaps(options?: {
  ...
  if (options) {
    ...
    if (options.content_type) {
      ...
      if (
        !VALID_MAP_CONTENT_TYPES.includes(
          options.content_type as MapContentType,
        )
      ) {
        throw new Error(
          `Invalid content_type: ${options.content_type}. Must be one of: ${
            VALID_MAP_CONTENT_TYPES.join(", ")
          }`,
        );
      }
