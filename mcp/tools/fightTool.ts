...
The tool accepts an optional character name.",
  schema: {
    character: z.string().default("Dexter").describe("The name of the character who will fight (default: Dexter)")
  },
  handler: async (args: { character: string }) => {
    try {
      ...
      if (response && response.error) {
        throw new Error(`Failed to fight: ${JSON.stringify(response.error)}`);
      }
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              ...
              details: response || "No response received"
            }, null, 2)
          }
        ]
      };
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred during combat';
